...
def convert_raw_data_to_hdf5(filename, dataDir, json_data, split):
    ...
    train_list_index, val_list_index, test_list_index = test_train_val_split(
        image_files, split
    )
    ...
    for idx, idx_ in tqdm(zip(train_image_files, train_label_files)):
        ...
        if data_augmentation:
            ...
            imgs_augmented, msks_augmented = dp.imbalanced_data_augmentation(
                imgs_, msk_
            )
        ...
        if first:
            ...
            img_train_dset = hdf_file.create_dataset(
                "imgs_train",
                imgs.shape,
                maxshape=(None, imgs.shape[1], imgs.shape[2], imgs.shape[3]),
                dtype=float,
            )
            img_train_dset[:] = imgs
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Medical AIC project data ",
        add_help=True,
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Resize height and width to this size. Original size = 512",
    )
    parser.add_argument(
        "--split", type=float, default=0.5, help="Train/test split ratio"
    )
