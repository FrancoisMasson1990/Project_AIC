...
class Button(object):
    ...
    def __init__(self,
        ...
        self.actor.GetActualPositionCoordinate()\
...
class Grid(object):
    ...
    def properties(self,
        ...
        if sizes[0] \
           and (sizes[1]/sizes[0] > limitRatio
                or sizes[2]/sizes[0] > limitRatio):
            sizes[0] = 0
            xtitle = ''
        if sizes[1] \
           and (sizes[0]/sizes[1] > limitRatio
                or sizes[2]/sizes[1] > limitRatio):
            sizes[1] = 0
            ytitle = ''
        if sizes[2] \
           and (sizes[0]/sizes[2] > limitRatio
        ...
        lines = []
        if xtitle:
            lines.append(shapes.Line([0, 0, 0], [1, 0, 0],
                                     c=xLineColor,
                                     lw=axesLineWidth))
        if ytitle:
            lines.append(shapes.Line([0, 0, 0], [0, 1, 0],
                                     c=yLineColor,
                                     lw=axesLineWidth))
        if ztitle:
            lines.append(shapes.Line([0, 0, 0], [0, 0, 1],
                                     c=zLineColor,
                                     lw=axesLineWidth))
        ...
        if xyGrid and xtitle and ytitle:
            gxy = \
                shapes.Grid(pos=(0.5, 0.5, 0),
                            normal=[0, 0, 1],
                            resx=rx,
                            resy=ry)
            gxy.alpha(xyAlpha).wireframe(xyGridTransparent)\
                .c(xyPlaneColor).lw(gridLineWidth).lc(xyGridColor)
            grids.append(gxy)
        if yzGrid and ytitle and ztitle:
            gyz = \
                shapes.Grid(pos=(0, 0.5, 0.5),
                            normal=[1, 0, 0],
                            resx=rz,
                            resy=ry)
            gyz.alpha(yzAlpha).wireframe(yzGridTransparent)\
                .c(yzPlaneColor).lw(gridLineWidth).lc(yzGridColor)
            grids.append(gyz)
        if zxGrid and ztitle and xtitle:
            gzx = \
                shapes.Grid(pos=(0.5, 0, 0.5),
                            normal=[0, 1, 0],
                            resx=rz,
                            resy=rx)
            gzx.alpha(zxAlpha).wireframe(zxGridTransparent)\
                .c(zxPlaneColor).lw(gridLineWidth).lc(zxGridColor)
            grids.append(gzx)

        grids2 = []
        if xyGrid2 and xtitle and ytitle:
            gxy2 = \
                shapes.Grid(pos=(0.5, 0.5, 1),
                            normal=[0, 0, 1],
                            resx=rx,
                            resy=ry)
            gxy2.alpha(xyAlpha).wireframe(xyGrid2Transparent)\
                .c(xyPlaneColor).lw(gridLineWidth).lc(xyGridColor)
            grids2.append(gxy2)
        if yzGrid2 and ytitle and ztitle:
            gyz2 = \
                shapes.Grid(pos=(1, 0.5, 0.5),
                            normal=[1, 0, 0],
                            resx=rz,
                            resy=ry)
            gyz2.alpha(yzAlpha).wireframe(yzGrid2Transparent)\
                .c(yzPlaneColor).lw(gridLineWidth).lc(yzGridColor)
            grids2.append(gyz2)
        if zxGrid2 and ztitle and xtitle:
            gzx2 = \
                shapes.Grid(pos=(0.5, 1, 0.5),
                            normal=[0, 1, 0],
                            resx=rz,
                            resy=rx)
            gzx2.alpha(zxAlpha).wireframe(zxGrid2Transparent)\
                .c(zxPlaneColor).lw(gridLineWidth).lc(zxGridColor)
            grids2.append(gzx2)
        ...
        if xyGrid and xtitle and ytitle:
            ...
            if yHighlightZero and min_bns[2] <= 0 and max_bns[3] > 0:
                ...
                hyx = shapes.Line([0, yhl, 0],
                                  [1, yhl, 0],
                                  c=yHighlightZeroColor)
        ...
        if yzGrid and ytitle and ztitle:
            ...
            if zHighlightZero and min_bns[4] <= 0 and max_bns[5] > 0:
                ...
                hzy = shapes.Line([0, 0, zhl],
                                  [0, 1, zhl],
                                  c=zHighlightZeroColor)
        ...
        if originMarkerSize:
            if xtitle:
                if min_bns[0] <= 0 and max_bns[1] > 0:  # mark x origin
                    ox = \
                        shapes.Cube([-min_bns[0] / sizes[0], 0, 0],
                                    side=originMarkerSize,
                                    c=xLineColor)
                    originmarks.append(
                        ox.lighting(specular=0,
                                    diffuse=0,
                                    ambient=1))
            ...
            if ytitle:
                if min_bns[2] <= 0 and max_bns[3] > 0:  # mark y origin
                    oy = \
                        shapes.Cube([0, -min_bns[2] / sizes[1], 0],
                                    side=originMarkerSize,
                                    c=yLineColor)
                    originmarks.append(
                        oy.lighting(specular=0,
                                    diffuse=0,
                                    ambient=1))
            ...
            if ztitle:
                if min_bns[4] <= 0 and max_bns[5] > 0:  # mark z origin
                    oz = \
                        shapes.Cube([0, 0, -min_bns[4] / sizes[2]],
                                    side=originMarkerSize,
                                    c=zLineColor)
                    originmarks.append(oz.lighting(specular=0,
                                                   diffuse=0,
                                                   ambient=1))
                ...
                for coo in range(1, rx):
                    ...
                    xds = \
                        shapes.Cylinder(v,
                                        ...
                                        axis=(1, 0, 0),
                                        res=10)
                    ticks.append(xds.c(xTickColor).lighting(specular=0,
                                                            ambient=1))
            if ytitle:
                for coo in range(1, ry):
                    ...
                    yds = \
                        shapes.Cylinder(v,
                                        ...
                                        axis=(0, 1, 0),
                                        res=10)
                    ticks.append(yds.c(yTickColor).lighting(specular=0,
                                                            ambient=1))
            if ztitle:
                for coo in range(1, rz):
                    ...
                    zds = \
                        shapes.Cylinder(v,
                                        ...
                                        axis=(0, 0, 1),
                                        res=10)
                    ticks.append(zds.c(zTickColor).lighting(specular=0,
                                                            ambient=1))
            ...
            for coo in range(1, rx*xMinorTicks):
                ...
                mxds = \
                    shapes.Cylinder(v,
                                    ...
                                    res=6)
                minorticks.append(
                    mxds.c(xTickColor).lighting(specular=0,
                                                ambient=1))
            ...
            for coo in range(1, ry*yMinorTicks):
                ...
                myds = \
                    shapes.Cylinder(v,
                                    ...
                                    res=6)
                minorticks.append(
                    myds.c(yTickColor).lighting(specular=0,
                                                ambient=1))
            ...
            for coo in range(1, rz*zMinorTicks):
                ...
                mzds = \
                    shapes.Cylinder(v,
                                    ...
                                    res=6)
                minorticks.append(
                    mzds.c(zTickColor).lighting(specular=0,
                                                ambient=1))
            ...
            if xtitle:
                ...
                for ic in range(1, rx+enableLastLabel):
                    ...
                    if abs(val) > 1 and sizes[0] < 1:
                        xLabelPrecision = \
                            int(xLabelPrecision-np.log10(sizes[0]))
                    ...
                    labels.append(xlab.c(xTickColor).lighting(specular=0,
                                                              ambient=1))
        if yLabelSize:
            if ytitle:
                if ry > 12:
                ...
                for ic in range(1, ry+enableLastLabel):
                    ...
                    if abs(val) > 1 and sizes[1] < 1:
                        yLabelPrecision = \
                            int(yLabelPrecision-np.log10(sizes[1]))
                    ...
                    ylab.pos(v)
                    labels.append(ylab.c(yTickColor).lighting(specular=0,
                                                              ambient=1))
        if zLabelSize:
            if ztitle:
                if rz > 12:
                ...
                for ic in range(1, rz+enableLastLabel):
                    ...
                    if abs(val) > 1 and sizes[2] < 1:
                        zLabelPrecision = \
                            int(zLabelPrecision-np.log10(sizes[2]))
                    zlab = \
                        shapes.Text3D(tval,
                                      pos=(0, 0, 0),
                                      s=zLabelSize,
                    ...
                    zlab.pos(v)
                    labels.append(zlab.c(zTickColor).lighting(specular=0,
                                                              ambient=1))
    ...
    def computeVisibleBounds(self, actors):
        ...
        if len(bns):
            ...
            max_bns = vbb
            min_bns = vbb
        sizes = np.array([max_bns[1]-min_bns[0],
                          max_bns[3]-min_bns[2],
                          max_bns[5]-min_bns[4]])
